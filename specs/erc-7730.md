...
Embedded calldata are used when a parameter of a smart contract contains the calldata of another function call to a smart contract.
This is common in contract interactions where one function call triggers another function call as part of its execution.
For example, the `permitAndCall` function that would verifie a permit and then call another function within the same contract using the provided calldata.

Here is an example of how to format embedded calldata in the descriptor JSON:
...
In this example, the `permitAndCall` function takes two parameters: `permit` and `action`.
The `action` parameter contains the calldata for another function call.
The format field is set to calldata, which tells the wallet to interpret the `action` parameter as embedded calldata.
The `calleePath` parameter specifies the path to the address of the contract being called, and the optional selector parameter can be used to specify the function selector if it is not the first 4 bytes of the calldata.
When displaying this transaction, the wallet will attempt to find an ERC-7730 descriptor for the embedded calldata using the callee address and the selector.

Sometimes, embedded calldata may require a transaction value and/or spender to be correctly clear signed, as the underlying function might spend a value passed from its parent transaction.
This can occur when the embedded calldata's associated function performs operations that involve the native currency value from the parent transaction, such as swaps or staking.
In such cases, `amount` and/or `spender` parameters can be used to specify those values.
...
| **`calldata`**           |                                                                                                                                                                                                                                                                                                                                                                                                                              |
|--------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| *Description*            | Data contains a call to another smart contract, or to another function of the same smart contract. To look for relevant ERC-7730 files matching this embedded calldata, use `callee` parameter and `selector`. If an ERC-7730 is not found or if embedded calldata are not supported by the wallet, it MAY display a hash of the embedded calldata instead, with target `calleePath` resolved to a trusted name if possible. |
| *Parameters*             | ---                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `selector`               | Optional selector, if not present the first 4 bytes of the calldata are interpreted as the selector                                                                                                                                                                                                                                                                                                                          |
| `calleePath` or `callee` | A path reference or constant value for the contract being called. If not present, the contract being called is interpreted as the same as the caller. The relevant ERC-7730 file matching this embedded calldata can refers this value using `@.to` container path.                                                                                                                                                          |
| `amount`                 | Optional associated amount in native currency, if the calldata can be associated with a container value. If set, the relevant ERC-7730 file matching this embedded calldata can refers this value using `@.value` container path.                                                                                                                                                                                            |
| `spender`                | Optional associated spender, if the calldata can be associated with a container value. If set, the relevant ERC-7730 file matching this embedded calldata can refers this value using `@.from` container path.                                                                                                                                                                                                               |
| *Examples*               |                                                                                                                                                                                                                                                                                                                                                                                                                              |
