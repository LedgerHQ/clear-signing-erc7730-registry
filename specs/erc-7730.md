...
In such cases, `amountPath` and/or `spenderPath` parameters can be used to specify those values.
...
| `selectorPath` or `selector` | Optional selector, if not present the first 4 bytes of the calldata are interpreted as the selector.  |
| `chainIdPath` or `chainId` | Optional chain ID if different from current contract.  |
| `amountPath` or `amount`  | Optional associated amount in native currency, if the calldata can be associated with a container value. If set, the relevant ERC-7730 file matching this embedded calldata can refers this value using `@.value` container path.|
| `spenderPath` or `spender`       | Optional associated spender, if the calldata can be associated with a container value. If set, the relevant ERC-7730 file matching this embedded calldata can refers this value using `@.from` container path.|
| *Examples*               |                                                                                                                                                                                                                                                                                                                                                                                                                              |
